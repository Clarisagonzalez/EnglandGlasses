...
const categoryIndex = urlParams.get("category") ?? 0;
const subcategoryIndex = urlParams.get("subcategory") ?? 0;
const selectedColor = urlParams.get("color") ?? null;
...
const category = categories[categoryIndex] ?? null;
const subcategory = category?.subcategories[subcategoryIndex] ?? null;

// Handle missing data gracefully
if (!category || !subcategory) {
  console.error("Category or subcategory not found.");
  alert("Invalid category or subcategory. Redirecting to shop.");
  window.location.href = "frames.html";
}

// Retrieve color details
const colorDetails = subcategory?.colors.find((color) => color.color === selectedColor) ?? null;

// Default frame details
const frameDetails = {
  title: subcategory?.title || "Unknown Frame Title",
  description: subcategory?.description || "No description available.",
  price: subcategory?.price || "$0.00",
  color: colorDetails?.color || "Default Color",
  src: colorDetails?.src || "images/default-image.jpg",
};
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  document.querySelector("#frame-description").innerText = frameDetails.description;
...
function savePrescription(event) {
  ...
  localStorage.setItem("selectedColor", frameDetails.color); // Save the selected color
