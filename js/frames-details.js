// Initialize selectedFrame with default values or fetch from localStorage, API, or other logic
let selectedFrame = {
  categoryIndex: 0,  // Default value
  subcategoryIndex: 0,  // Default value
  selectedColor: null  // No color selected by default
};
// Get the URL parameters for category, subcategory, and selected color
...
const category = categories[categoryIndex];
      ...
      // Log the selectedColor and the available colors for debugging
      console.log('Selected color:', selectedColor);
      console.log('Available colors:', subcategory.colors);
      // Find the selected color object from the subcategory
      const colorDetails = subcategory.colors.find(color => color.color === selectedColor);
      // If colorDetails is not found, alert and redirect
      if (!colorDetails) {
        console.log('Selected color details not found for:', selectedColor);
        alert('Selected color details not found.');
        window.location.href = 'index.html';  // Redirect if color details are not found
      }
      // Object to represent the frame details
      const frameDetails = {
        title: subcategory.title,
        description: subcategory.description,  // Ensure your subcategories have descriptions
        price: subcategory.price,
        color: selectedColor,
        src: colorDetails.src // Image source of the selected color
      };
      // Log the frame details to the console for debugging
      console.log('Frame details:', frameDetails);
      // Display the details on the page
document.addEventListener('DOMContentLoaded', function () {
      // Display the details on the page
      document.querySelector('#frame-title').innerText = frameDetails.title;
      document.querySelector('#frame-description').innerText = frameDetails.description || 'No description available';
      document.querySelector('#frame-image').src = frameDetails.src;
  ...
  function savePrescription(event) {
    ...
    const prescription = {
        sphOd: document.getElementById("sph-od").value,
        cylOd: document.getElementById("cyl-od").value,
        axisOd: document.getElementById("axis-od").value,
        pdOd: document.getElementById("pd-od").value,
        sphOs: document.getElementById("sph-os").value,
        cylOs: document.getElementById("cyl-os").value,
        axisOs: document.getElementById("axis-os").value,
        pdOs: document.getElementById("pd-os").value
    };
